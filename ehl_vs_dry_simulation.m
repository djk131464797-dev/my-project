...
%% --- Fig 5: Rolling Element Deformation vs Time ---
if exist('Data', 'var')
    % Reconstruct deformation for both models
    [delta_EHL, delta_impact_EHL] = ReconstructDeformation_EHL(Data(1).t, Data(1).y);
    [delta_DRY, delta_impact_DRY] = ReconstructDeformation_Dry(Data(2).t, Data(2).y);

    figure(5);
    set(gcf, 'Position', [0 100 1000 700], 'Color', 'w', 'Name', 'Rolling Element Deformation vs Time');

    % Time window for detail view
    t_start = 0.2;
    t_end = 0.5;
    idx1_def = (Data(1).t >= t_start) & (Data(1).t <= t_end);
    idx2_def = (Data(2).t >= t_start) & (Data(2).t <= t_end);

    % Subplot 1: EHL Deformation
    subplot(3,2,1);
    plot(Data(1).t(idx1_def), delta_EHL(idx1_def)*1e6, 'Color', Color_EHL, 'LineWidth', 1);
    title('(a) EHL Model: Rolling Element Deformation', 'FontName', std_font, 'FontSize', 11);
    ylabel('Deformation (\mu m)', 'FontName', std_font);
    grid off; box on;
    set(gca, 'FontName', std_font, 'FontSize', 10);

    % Subplot 2: Dry Contact Deformation
    subplot(3,2,2);
    plot(Data(2).t(idx2_def), delta_DRY(idx2_def)*1e6, 'Color', Color_Dry, 'LineWidth', 1);
    title('(b) Dry Contact Model: Rolling Element Deformation', 'FontName', std_font, 'FontSize', 11);
    ylabel('Deformation (\mu m)', 'FontName', std_font);
    grid off; box on;
    set(gca, 'FontName', std_font, 'FontSize', 10);

    % Calculate deformation rates (impact)
    d_delta_EHL = diff(delta_EHL) ./ diff(Data(1).t);
    d_delta_EHL = [d_delta_EHL; d_delta_EHL(end)];
    d_delta_DRY = diff(delta_DRY) ./ diff(Data(2).t);
    d_delta_DRY = [d_delta_DRY; d_delta_DRY(end)];

    % Subplot 3: EHL Deformation Rate
    subplot(3,2,3);
    plot(Data(1).t(idx1_def), d_delta_EHL(idx1_def), 'Color', Color_EHL, 'LineWidth', 1);
    title('(c) EHL Model: Deformation Rate (d\delta/dt)', 'FontName', std_font, 'FontSize', 11);
    ylabel('Deformation Rate (m/s)', 'FontName', std_font);
    grid off; box on;
    set(gca, 'FontName', std_font, 'FontSize', 10);

    % Subplot 4: Dry Contact Deformation Rate
    subplot(3,2,4);
    plot(Data(2).t(idx2_def), d_delta_DRY(idx2_def), 'Color', Color_Dry, 'LineWidth', 1);
    title('(d) Dry Contact Model: Deformation Rate (d\delta/dt)', 'FontName', std_font, 'FontSize', 11);
    ylabel('Deformation Rate (m/s)', 'FontName', std_font);
    grid off; box on;
    set(gca, 'FontName', std_font, 'FontSize', 10);

    % Subplot 5: Deformation Comparison
    subplot(3,2,5);
    hold on;
    plot(Data(1).t(idx1_def), delta_EHL(idx1_def)*1e6, 'Color', Color_EHL, 'LineWidth', 1, 'DisplayName', 'EHL');
    plot(Data(2).t(idx2_def), delta_DRY(idx2_def)*1e6, 'Color', Color_Dry, 'LineStyle', '--', 'LineWidth', 1, 'DisplayName', 'Dry');
    legend('FontName', std_font, 'FontSize', 9);
    title('(e) Deformation Comparison', 'FontName', std_font, 'FontSize', 11);
    ylabel('Deformation (\mu m)', 'FontName', std_font);
    xlabel('Time (s)', 'FontName', std_font);
    grid off; box on;
    set(gca, 'FontName', std_font, 'FontSize', 10);
    ...
    % Subplot 6: Deformation Rate Comparison
    subplot(3,2,6);
    hold on;
    plot(Data(1).t(idx1_def), d_delta_EHL(idx1_def), 'Color', Color_EHL, 'LineWidth', 1, 'DisplayName', 'EHL');
    plot(Data(2).t(idx2_def), d_delta_DRY(idx2_def), 'Color', Color_Dry, 'LineStyle', '--', 'LineWidth', 1, 'DisplayName', 'Dry');
    legend('FontName', std_font, 'FontSize', 9);
    title('(f) Deformation Rate Comparison', 'FontName', std_font, 'FontSize', 11);
    ylabel('Deformation Rate (m/s)', 'FontName', std_font);
    xlabel('Time (s)', 'FontName', std_font);
    grid off; box on;
    set(gca, 'FontName', std_font, 'FontSize', 10);
